...
const ProductsGrid: React.FC = () => {
	...
	const { priceFrom, priceTo, selectedSuppliers, setSelectedSuppliers, filtersApplied, applyFilters } = useFilterStore()
	...
	const handleRemoveSupplier = (supplier: string) => {
		setSelectedSuppliers(prevSelected => prevSelected.filter(s => s !== supplier))
		applyFilters()
	}
	...
	return (
		<main className='xl:px-52 px-5'>
			<div className='flex flex-col md:flex-row gap-x-5 mb-5 items-center gap-y-2 md:gap-y-0'>
				...
				/>
				<Button onClick={() => setSearchBy(search)} type='submit' className='w-full md:w-auto'>
					Szukaj
				</Button>
