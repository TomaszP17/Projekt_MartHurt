...
export default function Page() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await api.get(`http://localhost:8080/lightings/${slug}`);
        setProduct(res.data);
      } catch (err) {
  ...
  return (
    <main className="md:px-52 px-5">
      <Card className="drop-shadow-md relative overflow-hidden">
        <CardHeader>
          ...
          <Link href={"/lightings"}>
            <Button className="w-16 absolute top-4 left-0 mt-4 ml-4">
              Wróć
            </Button>
          </Link>
        ...
        <CardContent>
          <p className="text-lg font-semibold">
            Cena netto: {product.nettoClientBuyPrice} zł
          </p>
          <p className="text-lg font-semibold">
            Cena brutto: {product.bruttoClientBuyPrice} zł
          </p>
          <Table>
            <TableBody>
              <TableRow>
